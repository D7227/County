...
import { useScrapeItems, useUpdateItemStatus, useDeleteItem, useDeleteUpload, usePartyScrape, useStartAll } from "@/hooks/use-scraping";
...
export default function PartyPage() {
    ...
    const handleStartTask = (id: number) => {
        ...
        setLoadingId(id);
        partyScrape.mutate({ id, selectedVariations: variations }, {
            onSuccess: (data: any) => {
                toast({
                    title: "Party Scrape Triggered",
    ...
    const handleStartAll = () => {
        ...
        startBatch("party", itemsToScrape, selectedVariations);
    };
    ...
    return (
        <div className="space-y-6 h-[calc(100vh-4rem)] flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 flex-none">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-violet-600 to-purple-600 bg-clip-text text-transparent">Party Scrape</h2>
                    <p className="text-muted-foreground mt-3 font-medium flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-violet-500 animate-pulse" />
                        Manage and monitor party-based scraping operations.
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    {isBatching && batchType === 'party' ? (
                        <button
            ...
            <div className="flex flex-col md:flex-row gap-3 p-3 glass rounded-2xl flex-none border-white/20">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full h-10 pl-10 pr-4 py-2 rounded-xl bg-background border border-border/60 focus:ring-2 focus:ring-violet-400/20 focus:border-violet-400 transition-all text-sm"
                    />
                </div>
                <div className="flex items-center gap-2">
                    <select
                        ...
                        className="h-10 px-3 rounded-xl bg-background border border-border/60 text-sm focus:ring-2 focus:ring-violet-400/20 focus:border-violet-400 transition-all min-w-[180px]"
                    >
            ...
            <div className="flex-1 overflow-hidden glass rounded-2xl flex flex-col border-white/20">
                {isLoading ? (
                    <div className="flex-1 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-violet-500" /></div>
                ) : (
                            ...
                            <tbody className="divide-y divide-border/40">
                                {filteredItems.map((item) => (
                                    <Fragment key={item.id}>
                                        <tr className={cn(
                                            "hover:bg-violet-500/[0.04] transition-all border-none text-foreground/90 group",
                                            expandedState?.id === item.id ? "bg-violet-500/[0.06]" : ""
                                        )}>
                                                ...
                                                return (
                                                    <td
                                                        ...
                                                        className={cn(
                                                            ...
                                                            isParty && hasVal ? "font-bold text-violet-600 cursor-pointer hover:bg-violet-500/5" : "",
                                                        ...
                                                        <div className="flex flex-col gap-0.5 mt-1 ml-1">
                                                            {(() => {
                                                                try {
                                                                    ...
                                                                    const downloadCount = res.total_downloaded;
                                                                    return (
                                                                        <>
                                            ...
                                            <td className="px-4 py-3 text-center">
                                                <div className="flex items-center justify-center gap-2">
                                                    {(loadingId === item.id || (isBatching && batchType === 'party' && currentId === item.id)) ? (
                                                        <div className="inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-white text-violet-600 text-[10px] font-bold uppercase border border-violet-200 shadow-sm animate-pulse ring-2 ring-violet-500/5">
                                                        ...
                                                        <button
                                                            ...
                                                            className="group/btn inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 text-white shadow-lg shadow-purple-500/20 hover:shadow-xl hover:shadow-purple-500/30 hover:-translate-y-0.5 active:scale-95 font-bold text-[10px] uppercase tracking-wider transition-all disabled:opacity-50 border border-white/10"
                                                        >
                                                            <span>Party Scrape</span>
                                                        </button>
                                                    )}
                                                    <button
                                        ...
                                        {expandedState?.id === item.id && (
                                            <tr className="bg-violet-500/[0.02] border-y border-violet-500/10">
                                                <td colSpan={headers.length + 3} className="px-12 py-6">
                                                    <div className="glass-darker rounded-2xl p-6 border-white/20 shadow-xl animate-in slide-in-from-top-4 duration-300">
                                                        <div className="flex items-center justify-between mb-6">
                                                            <div className="flex items-center gap-3">
                                                                <div className="p-2 bg-violet-500/10 rounded-xl text-violet-600"><UserPlus className="w-5 h-5" /></div>
                                                                <div>
                                                                    <h4 className="text-sm font-bold text-foreground">Perfect Variances</h4>
                                                                    <p className="text-[11px] text-muted-foreground font-medium">Verify variations for <span className="text-violet-600 font-bold">{expandedState.partyKey}</span></p>
                                                                </div>
                                                            </div>
                                                            <button onClick={() => setExpandedState(null)} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground hover:text-violet-600 transition-all p-2 px-3 rounded-lg bg-white/40 border border-white/10">Close</button>
                                                        </div>
                                                                    ...
                                                                    return (
                                                                        <div
                                                                            ...
                                                                            className={cn(
                                                                                ...
                                                                                isSelected ? "bg-violet-500/10 border-violet-500 shadow-lg scale-[1.02]" : "bg-white/50 border-white/20 hover:border-violet-400/40 hover:bg-white/70"
                                                                            )}
                                                                        ...
                                                                        >
                                                                            ...
                                                                            />
                                                                            <label className="text-sm font-bold text-foreground group-hover:text-violet-600 transition-colors cursor-pointer leading-tight select-none">
                                                                                {variation}
                                                                            </label>
                                                                        </div>
                                                                    );
            ...
            <AlertDialog open={!!deleteConfirm} onOpenChange={(open) => !open && setDeleteConfirm(null)}>
                <AlertDialogContent className="bg-white/90 backdrop-blur-2xl border-white/50 rounded-[2.5rem] p-10 max-w-md animate-in zoom-in-95 duration-300 shadow-[0_32px_64px_-16px_rgba(139,92,246,0.15)] overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-violet-500 via-purple-500 to-violet-600" />
                    <AlertDialogHeader>
